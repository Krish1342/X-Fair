...
async def list_recurring(user_id: str, db: Session = Depends(get_db)):
    """
    Get all recurring transactions for a user with optimized query limiting.
    Returns maximum 100 recurring transactions to prevent performance issues.
    """
...
async def create_recurring(
    user_id: str, item: RecSchema, db: Session = Depends(get_db)
):
    uid = safe_uid(user_id)
...
async def get_recurring(user_id: str, rec_id: int, db: Session = Depends(get_db)):
    ...
    r = (
        db.query(dbm.RecurringTransaction)
        .filter(
            dbm.RecurringTransaction.user_id == uid,
            dbm.RecurringTransaction.id == rec_id,
        )
        .first()
    )
    if not r:
...
async def update_recurring(
    user_id: str, rec_id: int, item: RecSchema, db: Session = Depends(get_db)
):
    uid = safe_uid(user_id)
    r = (
        db.query(dbm.RecurringTransaction)
...
async def generate_due_transactions(
    user_id: str, up_to: Optional[str] = None, db: Session = Depends(get_db)
):
    ...
    db.commit()
    return {
        "message": "Generated transactions",
        "count": len(generated),
        "items": generated,
    }
...
async def preview_recurring(
    user_id: str, periods: int = 3, db: Session = Depends(get_db)
):
    uid = safe_uid(user_id)
    rec_list = (
        db.query(dbm.RecurringTransaction)
    ...
    for rec in rec_list:
        ...
        for _ in range(max(1, int(periods))):
            ...
            if rec.end_date and cur > rec.end_date:
                break
        previews.append(
            {"id": rec.id, "description": rec.description, "next_dates": dates}
        )
